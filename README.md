# Калькулятор для вычисления рациональных и комплексных чисел
...
Для запуска калькулятора в графическом представлении необходимо запустить файл **user.py**.

![mygif](images/calculator.gif)
...
Данный модуль предназначен для записи и хранения каждого действия пользователя. С помощью функции `info_log` мы записываем все действия(введенные цифры, операции, результат вычисления) выполненные пользователем в определенное время:
...
def info_log(val):           # информационные сообщения о рез-тах выполнения операций
    ...
    with open('log.txt', 'a') as file:
        file.write('{};Info:;{}\n'
                    .format(dt_now, val))
```
![Логирование](images/log.png)

**Результат записывается в отдельный файл:**


![Результат логирования](images/123.jpg)
...
**Полученное выражение от пользователя:**


![Полученное выражение от пользователя](images/до.jpg)


**Результат вычисления:**


![Результат вычисления](images/после.jpg)
...
![img input_str.png](images/input_str.png)
...
![img return_list_complex.png](images/return_list_complex.png)
...
![img return_calculator.png](images/retrn_calculator.png)

На выходе получаем строку с результатом, который подается в окно консоли калькулятора.

![img finish.png](images/finish.png)

![img return_answer.png](images/return_answer.png)


## Добавление ТГ- бота 

Также в качестве альтернативы взаимодействия с пользователем к проекту было добавлено также оперирование с телеграм-ботом ([Ссылка на ТГ-бот](t.me/GB_calculator_creating_bot)). Пользователь ввводит команду **/calc** и бот предлагает ввести ему численное выражение:

![ТГ-бот](images/tg.jpg)

Соответственно, бот сам определяет комплексные или рациональные нужно вычислить и после определения выдает пользователю уже готовый результат вычисления.


## Над программой работали:

**Модуль графического интерфейса для пользователя** - Маркач Алла Владимировна

**Модуль логирования** - Королева Татьяна Анатольевна

**Модуль вычисления рациональных чисел** - Воронина Милица Константиновна

**Модуль вычисления операций с комплексными и вещественными числами** - Старикова Екатерина Ивановна
